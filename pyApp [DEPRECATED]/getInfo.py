...
if os.path.exists("pyApp\config.json".strip()) == False:
    print("Config file not found, please run setup.py")
    input("Press enter to quit")
    quit()

with open("pyApp\config.json", "r") as f:
    config = json.load(f)
...
def getMusic():
    ...
    songInfo = [artist.strip(), songName.strip(), album.strip(), image.strip(), scrobbles]
